...
export default function LeadsPage() {
  ...
  const getStatusBadge = (st: LeadStatus) => {
    switch (st) {
      case 'new':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20">New</span>;
      case 'contacted':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-500/10 text-purple-400 border border-purple-500/20">Contacted</span>;
      case 'in_progress':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">In Progress</span>;
      case 'qualified':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Qualified</span>;
      case 'converted':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-500/10 text-green-400 border border-green-500/20">Converted</span>;
      case 'lost':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-500/10 text-slate-400 border border-slate-500/20">Lost</span>;
      default:
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-800 text-slate-300">{st}</span>;
    }
  };
  ...
  const getPriorityBadge = (pr: LeadPriority) => {
    switch (pr) {
      case 'high':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20">High</span>;
      case 'medium':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">Medium</span>;
      case 'low':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-slate-500/10 text-slate-400 border border-slate-500/20">Low</span>;
      default:
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-slate-800 text-slate-300">{pr}</span>;
    }
  };
  ...
  const formatFollowUpDisplay = (isoStr: string | null) => {
    if (!isoStr) return <span className="text-slate-500 text-xs italic">No follow-up set</span>;
    ...
    return (
      <div className={`flex flex-col text-xs ${isOverdue ? 'text-rose-400 font-medium' : 'text-slate-300'}`}>
        <span>{dateUs} {hour12}:{minute} {ampm}</span>
        {isOverdue && <span className="text-[10px] text-rose-500 font-semibold uppercase tracking-wider">Overdue</span>}
      </div>
    );
  };
    ...
    <DashboardLayout>
      <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800/80 pb-5">
          <div>
            <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Leads</h1>
            <p className="text-sm text-slate-400 mt-1">Manage prospects and follow-up opportunities</p>
          </div>
          <button
            onClick={handleOpenCreateModal}
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white text-sm font-semibold shadow-lg shadow-blue-500/10 transition-all active:scale-[0.98]"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
            <span className="text-xs text-blue-400 font-medium uppercase tracking-wider">New</span>
            <div className="text-2xl font-bold text-blue-300 mt-1">
              {metricsLoading ? '...' : metrics.newLeads}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
            <span className="text-xs text-cyan-400 font-medium uppercase tracking-wider">In Progress</span>
            <div className="text-2xl font-bold text-cyan-300 mt-1">
              {metricsLoading ? '...' : metrics.inProgressLeads}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
            <span className="text-xs text-emerald-400 font-medium uppercase tracking-wider">Qualified</span>
            <div className="text-2xl font-bold text-emerald-300 mt-1">
              {metricsLoading ? '...' : metrics.qualifiedLeads}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
            <span className="text-xs text-green-400 font-medium uppercase tracking-wider">Converted</span>
            <div className="text-2xl font-bold text-green-300 mt-1">
              {metricsLoading ? '...' : metrics.convertedLeads}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
            <span className="text-xs text-rose-400 font-medium uppercase tracking-wider">Follow-ups Due</span>
        ...
        {/* Filters Bar */}
        <form onSubmit={handleApplyFilters} className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
            {/* Search */}
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Search</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Product Interest</label>
              <input
                type="text"
                placeholder="Product..."
            ...
            {/* Follow-up Filter */}
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Follow-up</label>
              <select
          ...
          <div className="flex items-center justify-between border-t border-slate-850 pt-3">
            <span className="text-xs text-slate-400">
              Showing <strong className="text-slate-200">{leads.length}</strong> leads
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleClearFilters}
                className="px-3 py-1.5 rounded-lg border border-slate-800 hover:bg-slate-800 text-xs font-medium text-slate-400 hover:text-slate-200 transition-colors"
              >
                Clear Filters
              </button>
              <button
        ...
        {/* Desktop Dense Table */}
        <div className="hidden lg:block bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl">
                    ...
                    return (
                      <tr key={lead.id} className="hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 px-4">
                          <Link href={`/leads/${lead.id}`} className="font-semibold text-slate-100 hover:text-blue-400 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <span className="text-slate-300 font-medium">{lead.product_interest || 'N/A'}</span>
                        </td>
                        <td className="py-3 px-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {!isConverted && (
                              <select
                                ...
                                className="bg-slate-950 border border-slate-800 text-[11px] text-slate-300 rounded px-1.5 py-0.5 outline-none cursor-pointer hover:border-slate-700"
                              >
                                <option value="new">New</option>
                                <option value="contacted">Contacted</option>
                                <option value="in_progress">In Progress</option>
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            {!isConverted ? (
                              ...
                              <span className="px-2.5 py-1 rounded bg-slate-900 text-slate-600 text-[11px] font-medium cursor-not-allowed">
                                Edit
                              </span>
                            )}
                            ...
                            {!isConverted ? (
                              <button
                                ...
                                className="px-2.5 py-1 rounded bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 text-[11px] font-medium transition-colors border border-emerald-500/30"
                              >
                                Convert to Client
                              </button>
                              ...
                              <span className="px-2.5 py-1 rounded bg-slate-900 text-slate-600 text-[11px] font-medium border border-slate-800/40">
                                Converted
                              </span>
                            )}
        ...
        <div className="lg:hidden space-y-3">
          {loading ? (
            <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-6 text-center text-slate-500 text-xs">
              Loading leads...
            </div>
          ) : leads.length === 0 ? (
            <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-6 text-center text-slate-500 text-xs">
            ...
            leads.map((lead) => {
              const isConverted = lead.status === 'converted';
              return (
                <div key={lead.id} className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <Link href={`/leads/${lead.id}`} className="font-semibold text-slate-100 text-sm hover:text-blue-400">
                        {lead.first_name} {lead.last_name}
                      </Link>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs border-t border-b border-slate-800/60 py-2 text-slate-400">
                    <div>
                    ...
                    <div className="col-span-2">
                      <span className="text-[10px] text-slate-500 uppercase block font-semibold">Next Follow-up</span>
                      {formatFollowUpDisplay(lead.next_follow_up_at)}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1 flex-wrap">
                    <Link
                      href={`/leads/${lead.id}`}
                      className="flex-1 text-center py-1.5 rounded-lg bg-slate-800 text-slate-200 text-xs font-medium"
                    >
                      View
                    </Link>
                    {!isConverted ? (
                      <>
                        ...
                        <button
                          onClick={() => setConvertingLead(lead)}
                          className="px-3 py-1.5 rounded-lg bg-emerald-600/20 text-emerald-400 text-xs font-medium border border-emerald-500/30"
                        >
                          Convert
                        </button>
                      </>
                      ...
                      <Link
                        href={`/clients/${lead.converted_client_id}`}
                        className="px-3 py-1.5 rounded-lg bg-blue-600/20 text-blue-400 text-xs font-medium border border-blue-500/30"
                      >
                        Open Client
                      </Link>
                    ) : null}
                    {!isConverted && (
                      <button
                        onClick={() => setDeletingLead(lead)}
                        className="px-3 py-1.5 rounded-lg bg-rose-500/10 text-rose-400 text-xs font-medium border border-rose-500/20"
                      >
                        Delete
                      </button>
                    )}
