'use client';

import React, { useState, useEffect } from 'react';
import InlineEditActions from './InlineEditActions';
import { BASE_INLINE_SELECT_CLASSES, getInlineEditorWidthClass, InlineFieldType } from './editorStyles';

export interface SelectOption {
  label: string;
  value: string;
}

export interface InlineEditableSelectProps {
  value: string | null | undefined;
  options: SelectOption[];
  onSave: (newValue: string) => Promise<void> | void;
  label?: string;
  disabled?: boolean;
  emptyDisplay?: string;
  className?: string;
  selectClassName?: string;
  fieldType?: InlineFieldType | string;
}

export default function InlineEditableSelect({
  value,
  options,
  onSave,
  label,
  disabled = false,
  emptyDisplay = '—',
  className = '',
  selectClassName = '',
  fieldType = 'select',
}: InlineEditableSelectProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [draftValue, setDraftValue] = useState(value || '');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setDraftValue(value || '');
  }, [value]);

  const selectedOption = options.find((opt) => opt.value === value);

  const handleStartEdit = () => {
    if (disabled) return;
    setDraftValue(value || (options[0]?.value ?? ''));
    setError(null);
    setIsEditing(true);
  };

  const handleCancel = () => {
    setDraftValue(value || '');
    setError(null);
    setIsEditing(false);
  };

  const handleSave = async () => {
    if (saving) return;
    setSaving(true);
    setError(null);
    try {
      await onSave(draftValue);
      setIsEditing(false);
    } catch (err: any) {
      setError(err?.message || 'Failed to save');
    } finally {
      setSaving(false);
    }
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      handleSave();
    } else if (e.key === 'Escape') {
      e.preventDefault();
      handleCancel();
    }
  };

  return (
    <div className={`w-full font-sans ${className}`}>
      {label && <span className="block text-[15px] font-normal text-[#52627A] leading-snug mb-1">{label}</span>}

      {isEditing ? (
        <div className="flex items-center gap-2" onKeyDown={handleKeyDown}>
          <select
            value={draftValue}
            onChange={(e) => {
              setDraftValue(e.target.value);
              setError(null);
            }}
            disabled={saving}
            className={`${BASE_INLINE_SELECT_CLASSES} ${getInlineEditorWidthClass(fieldType)} ${selectClassName}`}
          >
            {options.map((opt) => (
              <option key={opt.value} value={opt.value}>
                {opt.label}
              </option>
            ))}
          </select>
          <InlineEditActions onSave={handleSave} onCancel={handleCancel} saving={saving} error={error} />
        </div>
      ) : (
        <div
          onClick={handleStartEdit}
          title={disabled ? undefined : 'Click to edit choice'}
          className={`group flex items-center justify-between py-1 px-2 -mx-2 rounded-lg transition-all ${
            disabled ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer hover:bg-slate-100/80 hover:text-blue-600'
          }`}
        >
          <span className="text-[15px] font-normal text-[#253247] leading-snug truncate">
            {selectedOption ? selectedOption.label : value || <span className="text-slate-400 font-normal italic">{emptyDisplay}</span>}
          </span>
          {!disabled && (
            <svg
              className="w-3.5 h-3.5 text-slate-350 opacity-0 group-hover:opacity-100 transition-opacity ml-1.5 shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
            </svg>
          )}
        </div>
      )}
    </div>
  );
}
