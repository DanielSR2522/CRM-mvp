...
export default function InlineEditableSSN({
  value,
  onSave,
  label,
  masked = false,
  ...
  const handleSave = async () => {
    if (saving) return;
    if (draftValue.trim() && !isValidSSNLength(draftValue)) {
  ...
  const displayFormatted = () => {
    ...
    return masked ? maskSSN(value) : formatSSN(value);
  };
    ...
    <div className={`w-full font-sans ${className}`}>
      ...
      {isEditing ? (
        ...
        <div
          ...
          title={disabled ? undefined : 'Click to edit SSN'}
